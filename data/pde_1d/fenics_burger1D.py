...
def burgers1d(run, nu, ng, dt, T, ng_out, save_dir, save_every=1, order=3, plot=False):
    ...
    mesh = df.UnitIntervalMesh(ng-1)
    mesh_out = df.UnitIntervalMesh(ng_out-1)
    ...
    # return u_save, run, time_taken
    np.save(args.save_dir + f'/raw{run}_mbt{2002}_tend{T}_nx{ng}.npy', u_save)
    # return u_save
...
if __name__ == '__maain__':
    ...
    parser.add_argument('--iend', type=int, default=15, help='start index (default: 200)')
    parser.add_argument('--processes', type=int, default=1, help='# processes (default: 4)')
    ...
    args.save_dir = f'./fenics_data'  # f'./fenics_data_dt{args.dt}_T{args.T}'
    ...
    # time_taken = np.array([p.get()[1:] for p in results])

    # np.save(args.save_dir+f'/ufenicsTimes_dt{args.dt}_T{args.T}.npy', time_taken)
    # len_data = len(data_[:, 0])
    # mbt = int(len_data / args.iend)
    # np.save(args.save_dir+f'/raw{len_data}_mbt{mbt}_tend{args.T}_nx{args.ng}.npy', data_)
    # print(time_taken)
    # print(data_.shape)
...
if __name__ == '__main__':
    ...
    for run in range(args.istart, args.iend):
        burgers1d(run, args.nu, args.ng, args.dt, args.T, args.ng_out,
                  args.save_dir, args.save_every, args.order, args.plot)

    # data_ = np.concatenate([burgers1d(run, args.nu, args.ng, args.dt, args.T, args.ng_out,
    #     args.save_dir, args.save_every, args.order, args.plot) for run in range(args.istart, args.iend)], 0)
    #
    # len_data = len(data_[:, 0])
    # mbt = int(len_data / args.iend)
    # np.save(args.save_dir+f'/raw{len_data}_mbt{mbt}_tend{args.T}_nx{args.ng}.npy', data_)
